...
# Simple demo script that creates test projects without requiring Rails environment
require 'fileutils'

puts "🚀 Setting up demo projects for Dev Assistant API..."
...
puts "\n📁 Projects created in /tmp/demo_projects/:"
demo_projects.each do |project|
  files = Dir.glob("#{project[:path]}/**/*").select { |f| File.file?(f) }
  puts "  - #{project[:name]}: #{files.length} files"
end

puts "\n🚀 Next steps to test the system:"
puts "1. Install bundle dependencies: bundle install"
puts "2. Start Ollama: ollama serve && ollama pull mistral && ollama pull nomic-embed-text"
puts "3. Start Rails server: rails server"
puts "4. Index these demo projects via API:"
puts "   curl -X POST http://localhost:3000/api/v1/index_project -d 'path=/tmp/demo_projects/blog_rails'"
puts "   curl -X POST http://localhost:3000/api/v1/index_project -d 'path=/tmp/demo_projects/react_todo'"
puts "   curl -X POST http://localhost:3000/api/v1/index_project -d 'path=/tmp/demo_projects/oauth_service'"
puts "5. Test queries via API:"
puts "   curl -X POST http://localhost:3000/api/v1/ask -d 'prompt=Which project used RAG?'"
puts "   curl -X POST http://localhost:3000/api/v1/ask -d 'prompt=What does .map do?'"
puts "   curl -X POST http://localhost:3000/api/v1/ask -d 'prompt=When was my last project with OAuth?'"

puts "\n🎉 Demo setup complete! The projects are ready for indexing."
